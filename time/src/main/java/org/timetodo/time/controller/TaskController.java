...
import org.timetodo.time.dto.TaskRequestDto;
import org.timetodo.time.entity.TaskEntity;
import org.timetodo.time.service.TaskService;
...
@RequestMapping("/task") // 이 컨트롤러의 모든 엔드포인트는 "/task" 경로로 시작됩니다.
public class TaskController {
    ...
    // 새로운 할 일을 추가하는 엔드포인트
    @PostMapping("/add")
    public ResponseEntity<TaskEntity> addTask(@ModelAttribute TaskRequestDto taskRequestDto) {
    ...
    public ResponseEntity<TaskEntity> updateTask(@PathVariable Long id, @ModelAttribute TaskRequestDto taskRequestDto) {
