...
//    @Column(name = "calendar_id", unique = true, nullable = false)
    ...
    private CategoryEntity categoryMTOcalendar; // Foreign Key (외래 키) - Category 클래스와 연관

    @ManyToOne // 여러 Calendar가 하나의 User와 연관 (N:1 관계)
    @JoinColumn(name = "user_id")
    private UserEntity userMTOcalendar; // 사용자 ID (Foreign Key)

    @OneToMany(mappedBy = "calendarMTOreminder")
