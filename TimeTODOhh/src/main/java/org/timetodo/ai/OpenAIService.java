...
public class OpenAIService { //사용자 입력 파실 및 처리
        ...
        requestBody.put("messages", List.of(
                Map.of("role", "system", "content",
                        "사용자의 요청을 JSON으로 처리하세요. 마지막 항목 뒤에 쉼표를 포함하지 마세요. 아래와 같은 구조를 따르십시오:\n" +
                                ...
                                "  * `status`는 보류중, 진행중, 완료 중 하나로 설정하세요.\n" +
                                "  * `repeatType`은 반복 없음, 매일, 매주, 매월, 매년 중 하나로 설정하세요.\n" +
    ...
    // 문자열을 LocalDateTime으로 안전하게 변환하는 헬퍼 메서드
    private LocalDateTime parseDateTime(String dateTimeStr, String fieldName) {
        if (dateTimeStr == null || dateTimeStr.trim().isEmpty()) {
            log.warn("{} 값이 null이거나 빈 문자열입니다. 기본 값을 설정합니다.", fieldName);
            return null; // 호출하는 곳에서 기본값 설정
        }
        try {
            return LocalDateTime.parse(dateTimeStr);
        } catch (DateTimeParseException e) {
            log.error("{} 값 변환 중 오류 발생: {}", fieldName, dateTimeStr, e);
            throw new IllegalArgumentException(fieldName + " 값이 올바르지 않습니다: " + dateTimeStr, e);
        }
    }
